  ...
  const getLabels = () => {
    if (locale === 'tg') {
      return {
        ...
        errorMsg: 'Бо сервери базаи маълумот алоқа барқарор карда наметавонистем. Боварӣ кунед, ки сервер бо фарманди npm run backend дар порти 3001 оғоз шудааст.',
      };
    } else if (locale === 'ru') {
      return {
        ...
        errorMsg: 'Не удалось подключиться к серверу базы данных. Убедитесь, что сервер запущен с помощью команды npm run backend на порту 3001.',
      };
    } else {
      return {
        ...
        errorMsg: 'Could not connect to database server. Make sure the server is running with npm run backend on port 3001.',
      };
    }
  };
  ...
  if (apartments.length === 0) {
    return (
      <div>
        <div className="catalog-content-header">
          <h2 className="catalog-title">{labels.allApartments}</h2>
          <span className="catalog-count">0</span>
        </div>
